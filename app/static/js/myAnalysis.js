(function(window) {
    ...
    function createMethodTab(type) {
        if (type === 'regression') {
            $("<label>", {
                ...
                name: "methodOptions",
                value: "linear"
            })).append("Multivariate Linear").appendTo($("#method_options_group"))
            $("<label>", {
                class: "btn btn-default btn-block"
            }).append($("<input>", {
                type: "radio",
                name: "methodOptions",
                value: "ridge"
            })).append("Ridge").appendTo($("#method_options_group"))
            $("<label>", {
                class: "btn btn-default btn-block"
            }).append($("<input>", {
                type: "radio",
    ...
    function attachChartWithButtons(data, labels, chartType, divId) {
        ...
        var row = $("<div/>", {
            ...
            }) //creates a row for the chart and buttons
        var col = $("<div/>", {
                class: 'col-md-11'
            }) //creates a column span 11/12 for chart
        $("<div/>", {
                id: chartId,
                class: 'container-fluid'
            }).appendTo(col) //create the Container Itself and append it
        col.appendTo(row)
    ...
    function attachChartButtons(divId, row, chartCount) {
        var btns_col = $("<div/>", {
                class: 'col-md-1'
            }) //creates a column span 1/12 for buttons
        var btn_group = $("<div/>", {
            class: 'btn-group-vertial'
        })
        if (chartCount % 2 == 0) {
            $("<button>", {
                id: "btn_close_" + chartCount,
